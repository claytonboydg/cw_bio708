...
v_abc100 <- rep(c("a", "b", "c"), c(20, 50, 30))
...
# Assign it to `m_num`.
m_num <- matrix(c(1:9), nrow = 3, ncol = 3)
...
df_mtcars <- cbind(df_mtcars, make = v_make)
...
df_mtcars %>% 
  ggplot(aes(x = f_cyl, y = wt, fill = f_cyl)) +
  geom_boxplot() +
